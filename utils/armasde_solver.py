...
def armasdeint(func_armaSDE, y0, ts):
    ...
    t = np.linspace(0, 1, nsteps+1)  # 時間軸
    ...
    y.append(y0)
    for k in range(1, nsteps + 1):
        y_next = y[k-1] + func_armaSDE.drift(y[k-1]) * dt \
            + func_armaSDE.diffusion(y[k-1]) * dB[:,k-1] # "dB[k]=B[k+1]-B[k]"    
        y.append(y_next) 
